...
### deploy_websocket.sh
# Compila, construye y sube la imagen Docker del servicio websocket (FastAPI) al registry privado
# Autor: Pedro / ChatGPT
# Fecha: 2025-06-04
...
@app.get("/")
def read_root():
    return {"status": "websocket online"}
EOF
fi
...
if [ ! -f "$ENV_FILE" ]; then
  ...
  echo "# Entorno websocket" > "$ENV_FILE"
fi
